...
export class Cif {
  ...
  setFormData(process: DetailProcess) {
...
export class CifCondition {
  ...
  dateCreatedValidator() {
    let start = moment(new Date(this.inputDateFrom))
    let end = moment(new Date(this.inputDateTo))
    let now = moment(new Date(moment().format('yyyy-MM-DD')))
    let condStart = start.diff(now, "day")
    let condEnd = end.diff(now, "day")
    ...
    if (!start.isValid() && !end.isValid()) {
      ...
      if (cond >= 0) {
        if (condStart > 0 && condEnd > 0) {
          ...
          if (this.inputDateTo != null && this.inputDateTo != '' && this.inputDateFrom != null && this.inputDateFrom != '') {
            if (cond > 366) {
